...
# a linear model. This shows results for an alternative model with delta_AIC<2 
# but not the best model overall. This model includes lat, long, and guild
...
create_CEI_model_and_predict <- function(df) {
  ...
  CEI_model <- lm(model_formula_CEI, data = df)
...
png("plots/SI/with_guild/CEI_lat.png", width = 3000, height = 3000, res = 1000)
...
png("plots/SI/with_guild/CEI_long.png", width = 3000, height = 3000, res = 1000)
...
create_LEI_model_and_predict <- function(df) {
  ...
  LEI_model <- lm(model_formula_LEI, data=df)
...
png("plots/SI/with_guild/LEI_lat.png", width = 3000, height = 3000, res = 1000)
...
png("plots/SI/with_guild/LEI_long.png", width = 3000, height = 3000, res = 1000)
...
# Load images
CEI_lat <- rasterGrob(readPNG("plots/SI/with_guild/CEI_lat.png"), interpolate = TRUE)
CEI_long <- rasterGrob(readPNG("plots/SI/with_guild/CEI_long.png"), interpolate = TRUE)
LEI_lat <- rasterGrob(readPNG("plots/SI/with_guild/LEI_lat.png"), interpolate = TRUE)
LEI_long <- rasterGrob(readPNG("plots/SI/with_guild/LEI_long.png"), interpolate = TRUE)
...
# save final plot
ggsave("plots/Figure_S14.png", final_plot, width = 12, height = 10)
...
create_predictions_by_guild <- function(df, model_name) {
  
  final_model <- lm(
    CEI ~ guild + mean_latitude + I(mean_latitude^2) + 
      mean_longitude + I(mean_longitude^2), 
    data = df,)
  
  # Create a data frame with distinct guild values and representative values for prediction
  guild_df <- df %>%
    distinct(guild) %>%
    mutate(
      mean_latitude = mean(df$mean_latitude, na.rm = TRUE),
  ...
  # Generate predictions with confidence intervals for each guild
  guild_predictions <- predict(final_model, newdata = guild_df, se.fit = TRUE)
  guild_df <- guild_df %>%
    mutate(
      fit = (guild_predictions$fit),
      lower = (guild_predictions$fit - 1.96 * guild_predictions$se.fit),
      upper = (guild_predictions$fit + 1.96 * guild_predictions$se.fit),
      model = model_name
    )
...
pred_2050_ssp2 <- create_predictions_by_guild(df_2050_ssp2, "2050 SSP2")
pred_2050_ssp5 <- create_predictions_by_guild(df_2050_ssp5, "2050 SSP5")
pred_2080_ssp2 <- create_predictions_by_guild(df_2080_ssp2, "2080 SSP2")
pred_2080_ssp5 <- create_predictions_by_guild(df_2080_ssp5, "2080 SSP5")
...
# Plotting side-by-side points and whiskers by Guild with gray shading
CEI_guild_plot_2050 <- ggplot(predictions_2050_df, aes(x = guild, y = fit, color = model)) +
  ...
  geom_segment(data = predictions_2050_df, aes(x = guild, xend = guild, y = 0.95, yend = 1.05), 
  ...
  labs(
    #title = "Predicted CEI by Guild across Scenarios",
    x = "Guild",
    y = "Predicted CEI",
    color = "Model"
  ) +
...
ggsave("plots/SI/with_guild/CEI_guild_2050.png", CEI_guild_plot_2050, width = 3.5, height = 3.5)



CEI_guild_plot_2080 <- ggplot(predictions_2080_df, aes(x = guild, y = fit, color = model)) +
  geom_point(position = position_dodge(width = 0.5), size = 3) +
  geom_errorbar(aes(ymin = lower, ymax = upper), width = 0.2, position = position_dodge(width = 0.5)) +
  scale_color_manual(values = model_colors) +
  geom_hline(yintercept = 1, linetype = "solid", color = "black", size = 0.5) +  # Add y=1 line
...
ggsave("plots/SI/with_guild/CEI_guild_2080.png", CEI_guild_plot_2080, width = 3.5, height = 3.5)
...
# -------------------------   LEI Predictions by guild for Each Scenario ---------------------------

# Define a function to generate predictions for a specific model and scenario
create_predictions_by_guild <- function(df, model_name) {
  
  final_model <- lm(
    LEI ~ guild + mean_latitude + I(mean_latitude^2) + 
      mean_longitude + I(mean_longitude^2), 
    data = df,)
  ...
  guild_df <- df %>%
    ...
    mutate(
      mean_latitude = mean(df$mean_latitude, na.rm = TRUE),
      mean_longitude = mean(df$mean_longitude, na.rm = TRUE),
      habitat = factor("Forests", levels = levels(df$forests)),
      order = factor("Passerine", levels = levels(df$order))
    )
...
# Plotting side-by-side points and whiskers by guild
LEI_guild_plot_2050 <- ggplot(predictions_2050_df, aes(x = guild, y = fit, color = model)) +
  geom_point(position = position_dodge(width = 0.5), size = 3) +
  geom_errorbar(aes(ymin = lower, ymax = upper), width = 0.2, position = position_dodge(width = 0.5)) +
  scale_color_manual(values = model_colors) +
  geom_hline(yintercept = 0.2, linetype = "solid", color = "black", size = 0.5) +  # Add y=1 line
  geom_segment(data = predictions_2050_df, aes(x = guild, xend = guild, y = 0.195, yend = 0.205), 
               inherit.aes = FALSE, color = "black", size = 0.3) + 
  geom_segment(aes(x = -Inf, xend = -Inf, y = 0.2, yend = 0.6), linetype = "solid", color = "black", size = 0.75) +  # Line at y=1
  labs(
    #title = "Predicted LEI by Guild across Scenarios",
    x = "Guild (2050's)",
    y = "Predicted LEI",
    color = "Model"
  ) +
...
ggsave("plots/SI/with_guild/LEI_guild_2050.png", LEI_guild_plot_2050, width = 3.5, height = 3.5)


# Plotting side-by-side points and whiskers by guild
LEI_guild_plot_2080 <- ggplot(predictions_2080_df, aes(x = guild, y = fit, color = model)) +
  geom_point(position = position_dodge(width = 0.5), size = 3) +
  geom_errorbar(aes(ymin = lower, ymax = upper), width = 0.2, position = position_dodge(width = 0.5)) +
  scale_color_manual(values = model_colors) +
  labs(
...
ggsave("plots/SI/with_guild/LEI_guild_2080.png", LEI_guild_plot_2080, width = 3.5, height = 3.5)
...
# Load images
CEI_guild_2050 <- rasterGrob(readPNG("plots/SI/with_guild/CEI_guild_2050.png"), interpolate = TRUE)
CEI_guild_2080 <- rasterGrob(readPNG("plots/SI/with_guild/CEI_guild_2080.png"), interpolate = TRUE)
LEI_guild_2050 <- rasterGrob(readPNG("plots/SI/with_guild/LEI_guild_2050.png"), interpolate = TRUE)
LEI_guild_2080 <- rasterGrob(readPNG("plots/SI/with_guild/LEI_guild_2080.png"), interpolate = TRUE)
glm_legend <- rasterGrob(readPNG("plots/glm_legend_reduced.png"), interpolate = TRUE)


# Create labeled plots using ggdraw
CEI_lat_labeled <- ggdraw() +
  draw_grob(CEI_guild_2050) +
  draw_label("a", x = 0.05, y = 0.99, hjust = 0, vjust = 1, size = 25, fontface = "bold") + 
  draw_label("2050", x = 0.45, y = 1, hjust = 0, vjust = 1, size = 25, fontface = "bold")

CEI_long_labeled <- ggdraw() +
  draw_grob(CEI_guild_2080) +
  draw_label("b", x = 0, y = 0.99, hjust = 0, vjust = 1, size = 25, fontface = "bold") + 
  draw_label("2080", x = 0.45, y = 1, hjust = 0, vjust = 1, size = 25, fontface = "bold")

LEI_lat_labeled <- ggdraw() +
  draw_grob(LEI_guild_2050) +
  draw_label("c", x = 0.05, y = 0.99, hjust = 0, vjust = 1, size = 25, fontface = "bold") +
  draw_label("2050", x = 0.45, y = 1, hjust = 0, vjust = 1, size = 25, fontface = "bold")

LEI_long_labeled <- ggdraw() +
  draw_grob(LEI_guild_2080) +
  draw_label("d", x = 0, y = 0.99, hjust = 0, vjust = 1, size = 25, fontface = "bold") + 
  draw_label("2080", x = 0.45, y = 1, hjust = 0, vjust = 1, size = 25, fontface = "bold")
...
# save final plot
ggsave("plots/Figure_S15.png", final_plot, width = 12, height = 12)
